...
def get_daily_profile_measurements_by_sensor(sensor_id, parameter_id, qc_level, from_date, to_date, order_by='DESC'):
    query = "SELECT * FROM daily_profile_measurements_by_sensor WHERE sensor_id=? AND parameter_id=? AND qc_level=? AND year=? AND date>=? AND date<=? ORDER BY date {order}".format(order=order_by)
    prepared = session.prepare(query)
